...
- 本机记录文本、图片与文件剪贴板历史，支持全局快捷面板、搜索、收藏、纯文本/合并/顺序粘贴和固定片段。
...
- [片段库与剪贴板历史 PRD](docs/PRD-片段库与剪贴板历史.md)：记录片段库、剪贴板采集、全局快捷面板和隐私规则的产品设计。
...
Tauri 会自动启动 Vite，并打开桌面窗口。Git 报告、密钥库、剪贴板监听与自动粘贴、原生目录选择和托盘交互需要桌面模式。macOS 首次使用剪贴板来源识别和自动粘贴时，需要在“系统设置 → 隐私与安全性 → 辅助功能”中授权；应用打开时若检测到未授权，会自动弹出系统授权对话框，也可在设置或剪贴板历史页点“去授权”再次唤起。
